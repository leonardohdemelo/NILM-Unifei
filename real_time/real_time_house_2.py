...
def processa_dados(modelo, line):
    ...
    list_cafeteira = predicao['Maquina'].tolist()

    arquivo = open('geladeira_casa_2.txt', 'w')
    arquivo.write(str(list_geladeira[len(list_geladeira)-1])+'\n')
    arquivo.close()
    
    arquivo = open('maquina_casa_2.txt', 'w')
...
print("[OK] Carregando base de dados da casa: " + HOUSE)
buffer = []
