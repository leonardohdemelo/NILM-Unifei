...
def processa_dados(modelo, line):
    ...
    predicao.columns = ['Geladeira', 'Maquina']
    list_geladeira = predicao['Geladeira'].tolist()
    list_maquina = predicao['Maquina'].tolist()
    ...
    pload2 = str(list_maquina[len(list_maquina) - 1])

    try:
        requests.post('http://unifeienergia.ml:1880/api/casa2/maquina', data=pload2, timeout=0.0000001)
    except requests.exceptions.ReadTimeout: 
        pass
    
    arquivo = open('geral_casa_2.txt', 'a') 
    arquivo.write(f'{str(start)} {str(line[-1].split()[1])}\n')
    arquivo.close()

    arquivo = open('geladeira_casa_2.txt', 'a')
    arquivo.write(f'{str(start)} {str(list_geladeira[len(list_geladeira)-1])}\n')
    arquivo.close()
    
    arquivo = open('maquina_casa_2.txt', 'a')
    arquivo.write(f'{str(start)} {str(list_maquina[len(list_maquina)-1])}\n')
    arquivo.close()
